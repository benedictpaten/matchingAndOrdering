...
static void referenceIntervalInsertion_fillOut(ReferenceIntervalInsertion *referenceIntervalInsertion,
        stIntTuple *chain, bool orientation, double score, ReferenceInterval *referenceInterval) {
...
static double referenceIntervalInsertion_isConnectedLeft(ReferenceIntervalInsertion *referenceIntervalInsertion,
        double *z, int32_t nodeNumber) {
...
static double referenceIntervalInsertion_isConnectedRight(ReferenceIntervalInsertion *referenceIntervalInsertion,
        double *z, int32_t nodeNumber) {
...
static double referenceIntervalInsertion_isCurrentlyConnected(ReferenceIntervalInsertion *referenceIntervalInsertion,
        double *z, int32_t nodeNumber) {
    assert(referenceIntervalInsertion->referenceInterval != NULL);
...
static double referenceIntervalInsertion_connectionScore(ReferenceIntervalInsertion *referenceIntervalInsertion,
        double *z, int32_t nodeNumber) {
        ...
        while (1) {
            if (positiveScores[j] > negativeScores[j]) {
                if (positiveScores[j] > maxReferenceIntervalInsertion.score) {
                    referenceIntervalInsertion_fillOut(&maxReferenceIntervalInsertion, chain, 1, positiveScores[j],
                            referenceInterval);
                }
            } else {
                if (negativeScores[j] > maxReferenceIntervalInsertion.score) {
                    referenceIntervalInsertion_fillOut(&maxReferenceIntervalInsertion, chain, 0, negativeScores[j],
                            referenceInterval);
                }
            }
    ...
    *totalScore = 0.0;
    ...
    while (stSortedSet_size(chains) > 0) {
        ...
        ReferenceIntervalInsertion maxReferenceIntervalInsertion = getMaxReferenceIntervalInsertion(reference,
                stSortedSet_getFirst(chains), z, nodeNumber);
        ...
        if (!fast) {
            ...
            while ((chain = stSortedSet_getNext(it2)) != NULL) {
                ReferenceIntervalInsertion maxReferenceIntervalInsertion2 = getMaxReferenceIntervalInsertion(reference,
                        chain, z, nodeNumber);
                assert(maxReferenceIntervalInsertion2.score != INT64_MIN);
                if (maxReferenceIntervalInsertion2.score > maxReferenceIntervalInsertion.score
                        || (maxReferenceIntervalInsertion2.score == maxReferenceIntervalInsertion.score
                                && referenceIntervalInsertion_connectionScore(&maxReferenceIntervalInsertion2, z,
                                        nodeNumber) > referenceIntervalInsertion_connectionScore(
    ...
    for (int32_t k = 0; k < permutations; k++) {
        ...
        for (int32_t i = 0; i < stList_length(chains); i++) {
            ...
            for (int32_t j = 0; j < l; j++) {
                ...
                connectionScores[j] = referenceIntervalInsertion_connectionScore(referenceIntervalInsertion, z,
                        nodeNumber);
            ...
            if (pureGreedy) {
                ...
                for (int32_t j = 0; j < l; j++) {
                    if (normalScores[j] > maxScore || (normalScores[j] == maxScore && connectionScores[j]
                            > maxConnectionScore)) {
